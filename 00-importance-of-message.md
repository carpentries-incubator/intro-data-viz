...
**TODO** Decide if this information needs to be here in this explicit way - could be that the chart ingredients get referenced along the way and summarized at the end.
...
A chart is a visual representation of the relationships among data elements that have been recorded in a table. A visual representation allows people to SEE (immediately comprehend) relationships among data elements by processing them in the brain's visual cortex (citation?) rather than reading relationships and processing them in the language center.[^1].
...
The relationship between the two elements that emerges when `x` is plotted against `y` creates the message of the chart.
...
a. Dogs get fewer daily walks during seasonal temperature extremes.
b. Monthly average temperature varies during the year.
c. Dogs get between 0 and 4 walks daily.
...
The correct answer is a. This sentence describes an interaction between seasonal temperature and number of daily walks.
...
In this lesson we will use a dataset about large North American canids: wolves, coyotes, and foxes[^2]. Public policy and scientific decisions have drastically changed these populations over the past century. The presence or absence of these animals near population centers, livestock, and nature preserves stirs public debate in the present day.
...
[^2]: Newsome, Thomas M.; Ripple, William J. (2015), Data from: A continental scale trophic cascade from wolves through coyotes to foxes, Dryad, Dataset, [https://doi.org/10.5061/dryad.s0d20](https://doi.org/10.5061/dryad.s0d20)
